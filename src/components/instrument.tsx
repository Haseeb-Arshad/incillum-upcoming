...
/**
 * The instrument — the only thing on this site that moves.
 *
 * ── What it is ─────────────────────────────────────────────────────────────
 *
 * A twenty-four hour rule with the staffed day marked on it at true scale, and
 * a mark at the reader's own local time.
 *
 ...
 * ── Where it sits now ──────────────────────────────────────────────────────
 *
 * It used to be a dark panel on a light page, and it carried its own
 * `data-inverted` and its own border and radius to say so. The page has a night
 * in the middle of it now, and this plate is inside it — so the panel came off.
 * What is left is the drawing on the band's own ground, which is what it should
 * always have been: the instrument is a measurement of the night, and framing
 * it made it a picture of one.
 *
 * Nothing in here knows it is dark. Every value is `text-ink`, `border-line` or
 * `bg-ink/…`, and the band above re-points those; the plate would render
 * correctly on paper if it were moved.
 *
 ...
 * ── The mark is lit ────────────────────────────────────────────────────────
 *
 * The reader's hour carries the site's one colour. It is the only live element
 * on the page and it is inside a band about a building at night, so the thing
 * that says *now* is the thing that is still on — the same light as the lamp in
 * the plate above and the eleven windows behind it.
 *
 * It is not a claim. Nothing here says Incillum is doing anything at that hour,
 * for that reader, anywhere, and nothing here may ever be made to: no "active
 * now", no counter, no pulse. A mark that means *this is your hour* is a
 * measurement; the same mark with a verb attached to it is a lie that would be
 * told to every visitor simultaneously.
 *
 * ── Server rendering ───────────────────────────────────────────────────────
 *
...
export function Instrument() {
  ...
  return (
    <section aria-label="A day, drawn to scale" className="pb-20 sm:pb-28">
      <Container>
        <Reveal>
          <div>
            {/*
          ...
          */}
            <div aria-hidden="true">
              {/*
                The readout rides above the mark, and carries only the time.

                ── Why the zone is not up here ──────────────────────────────

                It was, and it was a latent layout fault. The block is centred
                on the mark and clamped to 12–88 per cent, and a clamp in per
                cent can only be safe if the thing being clamped has a known
                width. `23:55` does — five glyphs in the tabular figures. An
                IANA zone does not: `Europe/London` is a hundred pixels and
                `America/Argentina/Buenos_Aires` is more than twice that, so at
                390px, near midnight, a reader in the wrong timezone got three
                pixels of horizontal scroll on the entire document. Nobody tests
                at 23:55 from Buenos Aires.

                So the zone moved to the key row under the plate, opposite the
                legend — which is where a technical annotation belongs anyway,
                and it can be any width there because the row is a flex
                `justify-between` rather than a point on a scale.

                The mark itself is never clamped. The mark is the measurement,
                and moving it would make the drawing lie.
              */}
              <div className="relative h-[4.5rem] sm:h-20">
                <div
                  className="absolute bottom-0 -translate-x-1/2 transition-opacity duration-[520ms]"
                  style={{
                    left: clock
                      ? `${Math.min(Math.max((clock.hours / DAY_HOURS) * 100, 12), 88)}%`
                      : '50%',
                    opacity: clock ? 1 : 0,
                  }}
                >
                  <span className="ic-tabular text-[2.25rem] leading-none text-signal sm:text-[2.75rem]">
                    {clock?.time ?? '00:00'}
                  </span>
                </div>
              </div>
              ...
              {/*
                `overflow-hidden` because of the mark's cap.

                The mark is a hairline at the reader's own hour, and it is
                capped with a 5px square that overhangs it by two pixels on the
                left. Between about 23:50 and midnight the line sits at 99.x per
                cent and the cap hangs three pixels past the rule — which at
                390px is three pixels of horizontal scroll on the whole
                document, for roughly ten minutes of the day, which is the kind
                of fault that ships because nobody tests at 23:55.

                Clipping is also the correct drawing: the rule is twenty-four
                hours wide and the mark cannot be outside the day.
              */}
              <div className="relative h-24 overflow-hidden border-y border-line sm:h-28">
                {/*
              ...
              */}
                <span
                  className="absolute inset-y-0 w-px bg-signal transition-opacity duration-[520ms]"
                  style={{
                    left: clock ? percent(clock.hours) : '50%',
                    opacity: clock ? 1 : 0,
                  }}
                >
                  <span className="absolute -top-px -left-[2px] size-[5px] bg-signal" />
                </span>
              </div>
              ...
              */}
              <div className="mt-6 flex items-center justify-between gap-4">
                ...
                {/*
                  The reader's zone, printed because a bare time invites the
                  suspicion that it was invented. Empty until the browser has
                  one, and empty for good on an engine that will not say — the
                  time is still correct and still worth showing, and only the
                  label is dropped.
                */}
                <span className="text-label uppercase text-ink-400">{clock?.zone ?? ''}</span>
              </div>
            </div>
          ...
          */}
            <p className="mt-8 max-w-[58ch] text-lede text-ink sm:mt-10">
              {instrument.caption}{' '}
              {clock ? (
